...
Built a full-stack multi-platform publishing system with Spring Boot and React, covering JWT auth, user-isolated content management, platform-specific adaptation, reliable outbox-based publish dispatch, retry handling, platform capability modeling, and scheduled publishing.

Resume bullets:

- Designed and implemented a Spring Boot publishing workflow with publish batches, per-platform tasks, idempotent request handling, outbox-based reliable dispatch, and retryable task execution.
- Built a platform capability matrix to model API/manual/client-assisted publishing modes, auth requirements, scheduling support, status query support, and media upload support across WeChat, Zhihu, Bilibili, and Xiaohongshu.
- Added scheduled publishing by storing future publish times on batches/tasks and reusing the outbox due-time mechanism to dispatch tasks only after their scheduled timestamp.
- Developed a React + TypeScript frontend for draft management, platform adaptation preview/editing, publish batch creation, schedule selection, and task status tracking.
- Added integration tests covering auth, user isolation, content adaptation, publish success, retry behavior, capability matrix output, and scheduled publish validation.

## Current Limitations

- Real third-party platform APIs are not connected yet; publishing uses `MockPublisher`.
- Redis is included as infrastructure dependency but is not central to the current local flow.
- Scheduled publishing is supported at the backend and publish form level; a dedicated calendar view can be added as a future UI improvement.
- Database schema is currently managed by Hibernate `ddl-auto=update`; production deployments should use Flyway or Liquibase migrations.
